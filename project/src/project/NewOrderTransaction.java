...
public class NewOrderTransaction {
	...
	public NewOrderTransaction(Connection connection) {
		...
		getCustomerQuery = session.prepare("SELECT c_discount, c_last, c_credit FROM customer WHERE c_w_id = ? AND c_d_id = ? AND c_id =?;");
	}
	...
	public void newOrder(int cWID, int cDID, int cID, int numOfItems, int[] itemNumberArr, int[] sWarehouseID,
		...
		for (int i = 0; i < numOfItems; i++) {
			...
			session.execute(newOrderLineQuery.bind(cWID, cDID, nextOrderID, i, itemNumberArr[i], itemAmt,
					sWarehouseID[i], quantities[i], "S_DIST_" + district, i_name));
		
			arrayListOutput.add(String.format("Item: %d | %s, Warehouse %d. Quantity: %d. Aount: %.2f, Stock: %d", itemNumberArr[i], i_name, sWarehouseID, quantities[i], itemAmt, adjustedQty));
		}
		...
		session.execute(newOrderQuery.bind(cWID, cDID, nextOrderID, cID, null, numOfItems, allLocal, orderDate, totalAmt, null));
